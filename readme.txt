...
* Integration settings are now plain on/off checkboxes: the WordPress core integrations (Login, Register, Password reset, Comments) were the last dropdowns, and the modes they offered disappeared with the spam filter. Existing settings convert automatically on upgrade — every legacy mode that showed or verified the captcha becomes "on". The two "shortcode" modes convert so that behaviour is preserved: HTML Forms' becomes "off" (a shortcode-placed widget is still verified by the integration itself), and Contact Form 7's becomes "on", which keeps verification enforced while the integration skips auto-injection for any form that already renders a widget.
