...
* Integration settings are now plain on/off checkboxes: the WordPress core integrations (Login, Register, Password reset, Comments) were the last dropdowns, and the modes they offered disappeared with the spam filter. Existing settings convert automatically on upgrade — every legacy mode that showed or verified the captcha becomes "on". Two conversions to re-check if you used them: HTML Forms' "shortcode" mode becomes "off" (a shortcode-placed widget is still verified, exactly as before), and Contact Form 7's "shortcode" mode becomes "on", which keeps verification enforced but now also auto-injects the widget into CF7 forms.
